...
use crate::operators::{self as OP, matmul_transb, rms_norm, silu, silu_two_dimension, tensor_add};
...
fn mlp(
    ...
    let mut hidden_states2 = Tensor::<f32>::default(&vec![4, 3]);
    silu_two_dimension(&mut hidden_states2, gate, up);
    //hidden = hidden @ down_weight.T
    matmul_transb(hidden_states, 0.0, &hidden_states2, w_down, 1.0);
    //residual = hidden + residual
    tensor_add(residual, &hidden_states);
}
